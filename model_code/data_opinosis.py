...
class Topic():
    ...
    def __init__(self, obj, pointer_generation=False, s_mask = 0.05, gold_len = MAX_GOLD_LEN):
        ...
        self.articles = []
        for i, article in enumerate(obj['articles']):
        ...
        word_num = 0
        max_word_len = 0
        self.articles = flatten(self.articles)
        self.articles = split_into_sentences(" ".join(self.articles))
        self.sent_count = len(self.articles)

        #Load adj matrix
        self.adjm = np.zeros((self.sent_count, self.sent_count))
        ...
        for i in range(self.sent_count):
            try:
                sent_words = self.articles[i].split()
            except:
        ...
        self.adjm += (0.5) * (sent_sim[0:self.sent_count][0:self.sent_count] - np.eye(self.sent_count))
        self.adjm = torch.FloatTensor(1/2 * (self.adjm + np.transpose(self.adjm)))

        self.mask = torch.from_numpy(mask)

        #Find the extractive summarization
        golden_text = " ".join(self.gold)
        ext_sum = ""
        ext_sum_indx = list()
        for num_sent in range(3):
            max_rouge = 0
            max_rouge_index = 0
            for i in range(len(self.articles)):
                t = ext_sum
                t += self.articles[i]
                try:
                    score = rouge.get_scores(t, golden_text)[0]['rouge-2']['f']
                except:
                    continue
                #print(score)
                if score > max_rouge:
                    max_rouge = score
                    max_rouge_index = i
            ext_sum_indx.append(max_rouge_index)
            ext_sum += self.articles[max_rouge_index]


        self.ext_class = torch.zeros((self.sent_count))
        for index in ext_sum_indx:
            self.ext_class[index] = 1.0
...
class Batch:
    def __init__(self, topics, id, gold_len = MAX_GOLD_LEN):
        ...
        self.ext_class = torch.stack([
                torch.FloatTensor(np.pad(x.ext_class,
                    pad_width=[(0,self.max_num_sent-x.sent_count)],
                    mode='constant', constant_values=0))
                for x in self.topics])  
...
class DataSet():
    ...
    def split_train_valid_test(self, ratio):
        ...
        f = open('~/Graphical-Summarization/sum_data/opinosis_split.json', 'w')
        split_list = []
