...
class Word_RNN(nn.Module):
    ...
    def __init__(self, d_input, d_output, cell_type='LSTM', pool="attn", bi=True):
        super(Word_RNN, self).__init__()
        self.cell_type = cell_type
        self.lstm = RNN(d_input=d_input, d_hidden=d_output, 
        n_layers=1, cell_type=cell_type, pooling=pool, bidirectional=bi)
    ...
    def forward(self, text, text_len, text_mask, initial=None, device=None, pool_skip=False):
        ...
        return self.lstm(text, text_len, text_mask, initial, device, pool_skip=pool_skip)
    def clear_time(self):
